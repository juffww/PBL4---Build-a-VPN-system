...
int SocketManager::createTCPSocket() {
    ...
    if (sock == -1)
        std::cerr << "[ERROR] Failed to create TCP socket: " << getLastError() << std::endl;
    return sock;
}
...
int SocketManager::createUDPSocket() {
    ...
    if (sock == -1)
        std::cerr << "[ERROR] Failed to create UDP socket: " << getLastError() << std::endl;
    return sock;
}
...
int SocketManager::sendData(int sock, const char* data, int length) {
    ...
    if (sent == -1)
        std::cerr << "[ERROR] Send failed: " << getLastError() << std::endl;
    return sent;
}
...
int SocketManager::receiveData(int sock, char* buffer, int bufferSize) {
    ...
    if (received == -1)
        std::cerr << "[ERROR] Receive failed: " << getLastError() << std::endl;
    return received;
}
